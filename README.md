...
## 13. Metrics (Loss, PSNR)

The trainer now logs PSNR (Peak Signal-to-Noise Ratio) derived directly from the per-pixel RGB L2 loss in linear space.

- Let loss = mean over pixels of ( (dR)^2 + (dG)^2 + (dB)^2 ), i.e. sum of channel squared errors (per pixel) averaged spatially.
- Per-channel MSE = loss / 3.
- PSNR (dB) = 10 * log10( MAX_I^2 / MSE_channel ) with MAX_I = 1 for normalized linear RGB, simplifying to 10 * log10(3 / loss).
- EMA (exponential moving average) of both loss and PSNR (smoothing factor 0.95) is reported to indicate converged trend vs instantaneous noise.

Interpretation:
- Early training: expect fast PSNR rise (5–10 dB gain) if alignment & exposure are good.
- Plateau: small <0.1 dB improvements over hundreds of iterations signal near-convergence for current resolution / Gaussian count.
- If PSNR stagnates < 15 dB on real scenes: likely orientation/exposure or color collapse → try Auto-align, reseed colors, or reduce learning rates.

Planned metrics:
- SSIM (structural similarity) – lightweight windowed approximation will be added behind a periodic (every N steps) trigger to limit CPU overhead.
- Active SH coefficient energy ratios (L2 vs L1) – to monitor overfitting or noise amplification in higher orders.
