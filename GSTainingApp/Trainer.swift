...
final class Trainer: ObservableObject {
    ...
    @Published var psnrHistory: [Float] = [] // instantaneous PSNR (dB) history
    ...
    // Hyper-params
    private var lr: Float = 1e-3 // base learning rate
    // Per-group multipliers (tunable)
    private var lrColorMul: Float = 1.0
    private var lrOpacityMul: Float = 0.5
    private var lrSigmaMul: Float = 0.5
    private var lrSHL1Mul: Float = 0.8
    private var lrSHL2Mul: Float = 0.8
    private var lrPosMul: Float = 0.2
    private var lrScaleMul: Float = 0.3
    // Cosine schedule params
    private var scheduleWarmupIters: Int = 500
    private var scheduleTotalIters: Int = 20000
    private var scheduleMinFactor: Float = 0.1
    @inline(__always) private func scheduledLR(_ base: Float, iter: Int) -> Float {
        if iter < scheduleWarmupIters { return base * Float(iter+1) / Float(max(1,scheduleWarmupIters)) }
        let t = min(1.0, Float(iter - scheduleWarmupIters) / Float(max(1, scheduleTotalIters - scheduleWarmupIters)))
        let cosv = 0.5 * (1 + cosf(Float.pi * t))
        let factor = scheduleMinFactor + (1 - scheduleMinFactor) * cosv
        return base * factor
    }
    // SH regularization (separate for L1 and L2)
    private var lambdaSHL1: Float = 1e-4
    private var lambdaSHL2Start: Float = 5e-4
    private var lambdaSHL2End: Float = 1.5e-4
    private var lambdaSHL2RampIters: Int = 4000
    @inline(__always) private func currentLambdaSHL2(iter: Int) -> Float {
        let t = min(1.0, Float(iter) / Float(max(1, lambdaSHL2RampIters)))
        // cosine ramp-down from start to end
        let w = 0.5 * (1 + cosf(Float.pi * t)) // 1 -> 0
        return lambdaSHL2End + (lambdaSHL2Start - lambdaSHL2End) * w
    }
    ...
    // Cosine + warmup scheduled learning rate and per-group multipliers
    let baseStepLR = scheduledLR(lr, iter: iteration)
    let lrColor = baseStepLR * lrColorMul
    let lrOpacity = baseStepLR * lrOpacityMul
    let lrSigma = baseStepLR * lrSigmaMul
    let lrSH1 = baseStepLR * lrSHL1Mul
    let lrSH2 = baseStepLR * lrSHL2Mul
    let lrPosScale = baseStepLR * lrPosMul
    let lrScaleA = baseStepLR * lrScaleMul
        for i in 0..<gaussians.count {
            var g = gaussians[i]
            // Gradients
            ...
            shG1Grads[i] = gradG1
            shB1Grads[i] = gradB1
            // Apply SH regularization directly to gradients (decoupled weight decay style)
            gradR1 += 2.0 * lambdaSHL1 * shR1[i]
            gradG1 += 2.0 * lambdaSHL1 * shG1[i]
            gradB1 += 2.0 * lambdaSHL1 * shB1[i]
            if enableSHL2 {
                let lam2 = currentLambdaSHL2(iter: iteration)
                // L2 has two SIMD4 blocks per channel (only first 6 coeffs meaningful)
                // Add gradient = 2 * lambda * coeff
                // We modify the grad arrays in-place so Adam sees regularized gradients.
                shR2_ex1_Grads[i].x += 2*lam2*shR2_ex1[i].x
                shR2_ex1_Grads[i].y += 2*lam2*shR2_ex1[i].y
                shR2_ex1_Grads[i].z += 2*lam2*shR2_ex1[i].z
                shR2_ex1_Grads[i].w += 2*lam2*shR2_ex1[i].w
                shR2_ex2_Grads[i].x += 2*lam2*shR2_ex2[i].x
                shR2_ex2_Grads[i].y += 2*lam2*shR2_ex2[i].y
                // z,w padding remain zero
                shG2_ex1_Grads[i].x += 2*lam2*shG2_ex1[i].x
                shG2_ex1_Grads[i].y += 2*lam2*shG2_ex1[i].y
                shG2_ex1_Grads[i].z += 2*lam2*shG2_ex1[i].z
                shG2_ex1_Grads[i].w += 2*lam2*shG2_ex1[i].w
                shG2_ex2_Grads[i].x += 2*lam2*shG2_ex2[i].x
                shG2_ex2_Grads[i].y += 2*lam2*shG2_ex2[i].y
                shB2_ex1_Grads[i].x += 2*lam2*shB2_ex1[i].x
                shB2_ex1_Grads[i].y += 2*lam2*shB2_ex1[i].y
                shB2_ex1_Grads[i].z += 2*lam2*shB2_ex1[i].z
                shB2_ex1_Grads[i].w += 2*lam2*shB2_ex1[i].w
                shB2_ex2_Grads[i].x += 2*lam2*shB2_ex2[i].x
                shB2_ex2_Grads[i].y += 2*lam2*shB2_ex2[i].y
            }
            // Moments
            ...
            let wd: Float = 1e-4
            var stepColor = lrColor * (mhatColor / denomColor + wd * g.color)
            ...
            var stepOpacity = lrOpacity * (mhatOpacity / denomOpacity + wd * g.opacity)
            stepOpacity = max(-maxStepOpacity, min(maxStepOpacity, stepOpacity))
            g.opacity -= stepOpacity

            g.sigma -= lrSigma * (mhatSigma / (sqrt(vhatSigma) + eps))
            ...
            var stepSX = lrScaleA * (mhatSX / (sqrt(vhatSX) + eps))
            var stepSY = lrScaleA * (mhatSY / (sqrt(vhatSY) + eps))
            var stepSZ = lrScaleA * (mhatSZ / (sqrt(vhatSZ) + eps))
            ...
            var stepR1 = lrSH1 * (mhatR1 / denomR1 + wdSH * shR1[i])
            var stepG1 = lrSH1 * (mhatG1 / denomG1 + wdSH * shG1[i])
            var stepB1 = lrSH1 * (mhatB1 / denomB1 + wdSH * shB1[i])
            ...
            if enableSHL2 {
                ...
                adamUpdateSIMD4(beta1: beta1, beta2: beta2, eps: eps, grad: shR2_ex1_Grads[i], m: &mShR2_ex1[i], v: &vShR2_ex1[i], param: &shR2_ex1[i], lr: lrSH2, t: tt, clip: 0.05)
                adamUpdateSIMD4(beta1: beta1, beta2: beta2, eps: eps, grad: shR2_ex2_Grads[i], m: &mShR2_ex2[i], v: &mShR2_ex2[i], param: &shR2_ex2[i], lr: lrSH2, t: tt, clip: 0.05)
                adamUpdateSIMD4(beta1: beta1, beta2: beta2, eps: eps, grad: shG2_ex1_Grads[i], m: &mShG2_ex1[i], v: &mShG2_ex1[i], param: &shG2_ex1[i], lr: lrSH2, t: tt, clip: 0.05)
                adamUpdateSIMD4(beta1: beta1, beta2: beta2, eps: eps, grad: shG2_ex2_Grads[i], m: &mShG2_ex2[i], v: &mShG2_ex2[i], param: &shG2_ex2[i], lr: lrSH2, t: tt, clip: 0.05)
                adamUpdateSIMD4(beta1: beta1, beta2: beta2, eps: eps, grad: shB2_ex1_Grads[i], m: &mShB2_ex1[i], v: &mShB2_ex1[i], param: &shB2_ex1[i], lr: lrSH2, t: tt, clip: 0.05)
                adamUpdateSIMD4(beta1: beta1, beta2: beta2, eps: eps, grad: shB2_ex2_Grads[i], m: &mShB2_ex2[i], v: &mShB2_ex2[i], param: &shB2_ex2[i], lr: lrSH2, t: tt, clip: 0.05)
            }
            ...
            if geoOptimizePositions {
                ...
                let adj = posLr * (lrPosScale / max(1e-8, lr))
                var stepP = adj * (mhatP / (SIMD3<Float>(sqrt(vhatP.x), sqrt(vhatP.y), sqrt(vhatP.z)) + SIMD3<Float>(repeating: eps)))
                // clip for stability
    ...
    // Derive PSNR from loss. Our loss is per-pixel sum of squared RGB error (not averaged over channels), so per-channel MSE = loss/3.
    // Using MAX_I = 1.0 (linear space). Thus PSNR = 10*log10(1 / (loss/3)) = 10*log10(3 / loss).
    let psnr: Float = (loss > 0) ? (10.0 * log10f(3.0 / max(loss, 1e-12))) : 99.0
        // Update EMA loss for logging
        lossEMA = (lossEMA == 0) ? loss : (0.95 * lossEMA + 0.05 * loss)
    psnrEMA = (psnrEMA == 0) ? psnr : (0.95 * psnrEMA + 0.05 * psnr)
        ...
        if iteration % 5 == 0 {
            ...
            log(String(format: "step=%d loss=%.6f (ema=%.6f) PSNR=%.2f dB (ema=%.2f) meanRGB=(%.3f, %.3f, %.3f) nonBlack=%.1f%% avgOpacity=%.3f var=%.5f gVar=%.5f sat=%.3f | reproj: front=%d(%.1f%%) inBounds=%d(%.1f%%)",
                       iteration,
                       loss, lossEMA,
                       psnr, psnrEMA,
